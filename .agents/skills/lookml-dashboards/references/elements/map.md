...
| :--- | :--- | :--- | :--- |
| `draw_map_labels_above_data` | `boolean (true/false)` | `Draw Map Labels Above Data` | Render map region/city labels on top of data layers |
| `elevation_scale` | `number` | `Elevation Scale` | Multiplier for 3D elevation height in 3D heatmap plots |
| `heatmap_gridlines` | `boolean (true/false)` | `Heatmap Gridlines` | Toggle visibility of gridlines on heatmaps |
| `heatmap_gridlines_empty` | `boolean (true/false)` | `Gridlines on Empty Regions` | Render gridlines over areas without data points |
...
| `hide_legend` | `boolean (true/false)` | `Hide Legend` | Toggle option to hide map legend |
| `map_bicycling_layer` | `boolean (true/false)` | `Bicycling Layer` | Show Google Maps bicycling overlay |
| `map_dual_axis` | `boolean (true/false)` | `Dual-axis Map` | Enable dual-metric map rendering |
| `map_kml_layer` | `boolean (true/false)` | `KML Layer` | Enable custom KML overlay layer |
...
| `map_pannable` | `boolean (true/false)` | `Allow Panning` | Allow user to drag and pan map |
...
| `map_traffic_layer` | `boolean (true/false)` | `Traffic Layer` | Show Google Maps real-time traffic layer |
| `map_transit_layer` | `boolean (true/false)` | `Transit Layer` | Show Google Maps transit line overlay |
...
| `map_zoomable` | `boolean (true/false)` | `Allow Zooming` | Allow user to zoom in and out |
| `quantize_map_value_colors` | `boolean (true/false)` | `Quantize Colors` | Group continuous values into discrete color bands |
| `reverse_map_value_colors` | `boolean (true/false)` | `Reverse Color Scale` | Invert direction of value color gradient |
| `series_colors.<series_id>` | `string` | `Color Overrides` | Custom hex color for a specific map layer |
| `show_legend` | `boolean (true/false)` | `Show Legend` | Show or hide map legend |
| `show_region_field` | `boolean (true/false)` | `Show Region Field in Tooltip` | Display region field inside hover tooltip |
| `show_view_names` | `boolean (true/false)` | `Show Full Field Name` | Show full field name in tooltip |
