...
> [!NOTE]
> Single Value visualization elements (`type: single_value`) do **not** use `modern2026: true`.
...
| `comparison_reverse_colors` | `boolean (true/false)` | `Positive Values are Bad` | Swap positive/negative indicator colors |
...
| `conditional_formatting_include_nulls_as_zero` | `boolean (true/false)` | `Include Null Values as Zero` | Treat null values as zero in conditional formatting |
